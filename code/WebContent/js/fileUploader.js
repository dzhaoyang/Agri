	...
	fileUploader = {
		...
		'init' : function(callback, options) {
			...
			if ($('#modal-fileUploader').length == 0) {
				$(document.body).append('<div id="modal-fileUploader" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true" style="top: -210px;MARGIN-RIGHT: auto;MARGIN-LEFT: auto;width: auto;background: rgba(0, 0, 0, 0.1)"></div>');
		...
		'init_crawler' : function() {/*
			...
			if(this.modalobj.find('#crawler').length == 0){
				this.modalobj.find('.nav-pills').append('<li role="presentation" style="display: none;"><a aria-controls="crawler" role="tab" data-toggle="tab" href="#crawler">提取网络文件</a></li>');
		...
		*/},
		...
		'reset_crawler' : function(){
			...
			this.modalobj.find('#crawler').find('.crawler-img').css("background-image","url('img/nopic.jpg')");
		},
		...
		'init_upload' : function(){
			...
			if(this.modalobj.find('#upload').length == 0){
				...
				var options = {
					...
					pick: {
						id: '#filePicker',//指定选择文件的按钮容器，不指定则不创建按钮。注意 这里虽然写的是 id, 但不只支持 id, 还支持 class, 或者 dom 节点
						label: '点击选择文件',
						multiple : $this.options.multi //是否开起同时选择多个文件能力
					},
					...
					swf: 'js/Uploader.swf',
					// 文件接收服务端。
					server: '/api/fileUploader/upload',//'./index.php?c=utility&a=file&do=upload',
					chunked: false,// 是否开起分片上传
					/*accept:{
						title: '图片',
					...
					compress: {//配置压缩的图片的选项
						//width: 1600,
					    //height: 1600,
						quality: 80,// 图片质量，只有type为`image/jpeg`的时候才有效。
						// 是否允许放大，如果想要生成小图的时候不失真，此选项应该设置为false.
					    //allowMagnify: false,
						// 是否允许裁剪。
					    //crop: false,
						preserveHeaders: true, // 是否保留头部meta信息。
						noCompressIfLarger: true,// 如果发现压缩后文件大小比原来还大，则使用原来图片,此属性可能会影响图片自动纠正功能
						compressSize: 1 * 1024 * 1024// 单位字节，如果图片大小小于此值，不会采用压缩。
					},
					formData : {
					...
					fileNumLimit: $this.options.multi ? 30 : 1,//验证文件总数量, 超出则不允许加入队列
					fileSizeLimit: 4 * 1024 * 1024,//验证文件总大小是否超出限制, 超出则不允许加入队列
					fileSingleSizeLimit: 30* 4 * 1024 * 1024//验证单个文件大小是否超出限制, 超出则不允许加入队列。
				}
