...
public class MediaAPI extends BaseAPI {
     ...
     * @return
     */
    private Media mediaUpload(String access_token,
                              MediaType mediaType,
                              Resource resource) {
     ...
     * @param mediaType
     * @param media
     *            多媒体文件有格式和大小限制，如下： 图片（image）: 128K，支持JPG格式
     *            语音（voice）：256K，播放长度不超过60s，支持AMR\MP3格式 视频（video）：1MB，支持MP4格式
     *            缩略图（thumb）：64KB，支持JPG格式
     * @return
     */
    public Media mediaUpload(String access_token,
                             MediaType mediaType,
                             File media) {
        return mediaUpload(access_token,
                           mediaType,
                           new FileSystemResource(media));
    }
     ...
     * @return
     */
    public ResponseEntity<ByteArrayResource> mediaGet(String access_token,
                                                      String media_id) {
