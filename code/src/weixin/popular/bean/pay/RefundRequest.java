...
public class RefundRequest {
    ...
    private String transaction_id; // 至少一个必填，同时存在时 transaction_id 优先 财付通订单号
                                   // transaction_id 财付通交易号, out_trade_no 和
                                   // transaction_id 至少一个必填，同时存在时 transaction_id
                                   // 优先
    ...
    private String recv_user_id; // 转账退款接收退款的财付通帐号。
                                 // 一般无需填写，只有退银行失败，资金转入商户号现金账号时（即状态为转入代发，查询返
                                 // 回的refund_status 是7 或11），填写原退款
                                 // 单号并填写此字段，资金才会退到指定财付通 账号。其他情况此字段忽略
    
    private String reccv_user_name; // 转账退款接收退款的姓名(需与接收退款的财 付通帐号绑定的姓名一致)
    
    private Integer use_spbill_no_flag;// 若通过接口(https://www.tenpay.com/cgi-bin/v1.0/pay_gate.cgi)
                                       // 支付的商户订单号来退款，则取值为1；而通过本文档支付接口的，则无需传值。
    
    private Integer refund_type; // 退款类型 为空或者填1:商户号余额退款；2：现金帐号退款；
                                 // 3:优先商户号退款，若商户号余额不足，再做现金帐号退款。使用2 或3
                                 // 时，需联系财付通开通此功能。
